...
describe('Sprints', () => {
  test('should not be created with user without a group', async () => {
    ...
    expect(res.body.error).toEqual(
      'Error creating sprint: User does not belong to any group or not found.',
    )
  })
  test('should not be created with incorrect dates', async () => {
    ...
    expect(resStartGreEnd.body.error).toEqual(
      'Error validating sprint: Start date must be before end date.',
    )

    expect(resInvalidDates.statusCode).toEqual(400)
    expect(Object.keys(resInvalidDates.body)).toContain('error')
    expect(resInvalidDates.body.error).toEqual(
      'Error validating sprint: Start date or end date is invalid.',
    )
  })
  test('should not be created with invalid sprint', async () => {
    ...
    expect(resInvalidSprint.body.error).toEqual(
      'Error validating sprint: Sprint must be a valid number.',
    )
  })
...
describe('GET /api/sprints', () => {
  ...
  test("should return only user's sprints when loggen in as an user", async () => {
    ...
    const res = await request(app).get('/api/sprints').set('Authorization', `bearer ${login.token}`)
...
describe('PUT /api/sprints', () => {
  ...
  test('should fail with missing fields', async () => {
    ...
    expect(resSprint.body.error).toEqual(
      'Error updating sprint: Start date or end date is invalid.',
    )
  ...
  test('should fail with dates overlapping with other sprints', async () => {
    ...
    expect(resEnd.body.error).toEqual(
      'Error updating sprint: End date is after the start date of the next sprint.',
    )

    expect(resStart.statusCode).toEqual(500)
    expect(Object.keys(resStart.body)).toContain('error')
    expect(resStart.body.error).toEqual(
      'Error updating sprint: Start date is before the end date of the previous sprint.',
    )
  ...
  test('should fail with pre-exising (excluding current) sprint number', async () => {
    ...
    expect(Object.keys(res.body)).toContain('error')
    expect(res.body.error).toEqual(
      'Error updating sprint: Invalid sprint number, sprint already exists.',
    )
