...
const create = async (req, res) => {
  ...
  try {
    const createdConfig = await db.RegistrationManagement.create(registrationManagement)
  ...
  try {
    ...
    if (!entry) {
      return res.status(400).json({
        error: 'There are no saved registration management configurations in database.',
      })
    }
