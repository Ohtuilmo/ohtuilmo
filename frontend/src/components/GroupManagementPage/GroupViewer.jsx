...
class GroupViewer extends React.Component {
  ...
  toggleEditMode = () => {
    ...
    if (this.state.editMode) {
      this.props.setSuccess(`Editing for group ${this.props.group.name} disabled!`)
    } else {
      this.props.setSuccess(`Editing for group ${this.props.group.name} enabled!`)
    }
  }

  render() {
    if (this.state.editMode) {
      return <SingleGroupEdit group={this.props.group} toggleEditMode={this.toggleEditMode} />
    } else {
      return (
        <div>
          <SingleGroupView
